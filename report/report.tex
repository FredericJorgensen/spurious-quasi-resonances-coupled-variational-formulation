...
\begin{remark}
    From here on, we will use the constant $\tilde c := \frac{c_i}{c_o}$ for two reasons. This reduces the amount of parameters we have to consider and
    makes it more clear that the resonance behavior depends only on the ratio and not the scaling
...
Since $U$ must satisfy  $(\Delta + \tilde c\kappa^2) U = 0$ from eq. \ref{eq:helmholtz}, 
we have 
$$
r^2 \partial_r^2V_n^r + r \partial_r V_n^r + (r^2  \tilde c \kappa^2 - n^2)V^r_n = 0.
$$
This is Bessel's equation. Since we require convergence at the origin we have $V^r_n(r) = J_n(\sqrt{\tilde}\kappa r)$.
...
$$
 = |v_n|^2 2\pi(1 + n^2) |J_n(\sqrt{ \tilde c} \kappa )|^2   \delta_{nm}  \overbrace{=}^! \delta_{nm}
$$
Therefore, we can choose real constants $v_n = \frac{1}{\sqrt{2 \pi (1 + n^2)} |J_n(\sqrt{ \tilde c} \kappa  )|}$.
...
We choose $$\theta_n =w_n e^{i n \phi}\text{ where } w_n = \frac{(1 + n^2)^{\frac{1}{4}}}{\sqrt{2 \pi}}$$ for $\mathcal{H}^{-\frac{1}{2}}(\Gamma)$ 
and for $\mathcal{H}^{1}(\Gamma)$ we choose  $$p_n = l_n e^{i n \phi} \text{ where } l_n = \frac{1}{\sqrt{2 \pi (1+n^2)}}. $$
...
For better comparability, let's construct the discretized version of the solution operator for the considered problem.
\begin{definition}
    Given positive real numbers \(k, c_{i}\), and \(c_{o}\), let \(S\left(c_{i}, c_{o}\right) \mathbf{f}:=\gamma_{C}^{-} u\)
    be the solution operator where \(u\) solves eq \ref{eq:helmholtz}.
\end{definition}
\begin{theorem}
According to paper 2006 BEM-FEM we can write $u$ as 
\begin{equation}
    \label{eq:boundary-integral-representation}
    u = -\Psi_{\mathrm{SL}}^{\kappa}\left(\left[\gamma_{N} u\right]_{\Gamma}\right)+\Psi_{\mathrm{DL}}^{\kappa}\left(\left[\gamma_{D} u\right]_{\Gamma}\right)
\end{equation}
where $\Psi_{SL}$ and $\Psi_{DL}$ are the single and double layer potential, respectively. 
Moreover, we used the notation \(\left[\gamma_{D} V\right]_{\Gamma}:=\gamma_{D}^{+} V-\gamma_{D}^{-} V, \quad\left[\gamma_{N} V\right]_{\Gamma}:=\gamma_{D}^{+} V-\gamma_{D}^{-} V\)
\end{theorem}
Note that eq. \ref{eq:helmholtz} implies that $\left[\gamma_{D} u\right] = f_1, \left[\gamma_{N} u\right] =  f_2$.

No let's use the following identity from the 2006 BEM-FEM paper.

\begin{align} 
    \gamma_{D}^{\pm} \Psi_{\mathrm{SL}}^{\kappa} &=\mathrm{V}_{\kappa}, & \gamma_{N}^{\pm} \Psi_{\mathrm{SL}}^{\kappa} &=\mathrm{K}_{\kappa} \pm \frac{1}{2} \mathrm{ld} \label{eq:bound-ops-identities}\\ 
    \gamma_{D}^{\pm} \Psi_{\mathrm{DL}}^{\kappa} &=\mathrm{K}_{\kappa}^{\prime} \mp \frac{1}{2} \mathrm{ld}, & \gamma_{N}^{\pm} \Psi_{\mathrm{DL}}^{\kappa} &=-\mathrm{W}_{\kappa}.  \nonumber
\end{align}

We'll plug this into \(\gamma_C^-u\) and obtain the solution operator: 
\begin{align}
    \gamma_C^-u & = \gamma_C^-\left(-\Psi_{\mathrm{SL}}^{\kappa}\left(f_2\right)+\Psi_{\mathrm{DL}}^{\kappa}\left(f_1\right) \right) = 
    \left(\gamma_D^-\left(-\Psi_{\mathrm{SL}}^{\kappa}\left(f_2\right)+\Psi_{\mathrm{DL}}^{\kappa}\left(f_1\right) \right),
    \gamma_N^- \left(-\Psi_{\mathrm{SL}}^{\kappa}\left(f_2\right)+\Psi_{\mathrm{DL}}^{\kappa}\left(f_1\right) \right)\right)  \nonumber \\
    & = \left( - V_\kappa f_2 + (K_\kappa^\prime + \frac{1}{2}) f_1, 
    -(K_\kappa -\frac{1}{2})f_2 - W_\kappa f_1 \right).
\end{align}

Now we will restrict the solution operator to $\mathcal{S}^{\frac{1}{2}} \times \mathcal{S}^{-\frac{1}{2}}$ 
and write it out in our orthonormal basis. Assuming we write out $\vec{f}$ and $u$ in our orthonormal basis, we can achieve this by simply plugging in the basis vectors for $f_1$ and $f_2$. 
This results in a blockdiagonal matrix with the following blocks.
\begin{equation}
    S_n =  
    \begin{pmatrix}
        \lambda_n^{(K')} + \frac{1}{2} & - \lambda_n^{(V)}  \\
        - \lambda_n^{(W)} & -(\lambda_n^{(K)} - \frac{1}{2})
    \end{pmatrix}
\end{equation}
